...
"""
The module provides an implementation of TGM using GPU
    ...
    def _fit_single(self, X, random_state, y=None):
        ...
        while change:
            ...
            with tf.device('/device:GPU:0'):
                    ...
                    a_sparse = tf.convert_to_tensor(B, np.float32)
                    b_sparse = tf.convert_to_tensor(Z_init, np.float32)
                    # a_sparse = tf.SparseTensor(indices, values, dense_shape)

                    c = tf.compat.v1.sparse_matmul(a_sparse, b_sparse)
                    print(tf.shape(c))
            ...
            with tf.device('/device:GPU:1'):
                np.random.seed(0)
                a_sparse = tf.convert_to_tensor(BZ, np.float32)
                b_sparse = tf.convert_to_tensor(Z.T, np.float32)
                c = tf.compat.v1.sparse_matmul(a_sparse, b_sparse)

                ktk = tf.compat.v1.sparse_matmul(b_sparse, a_sparse)
            ...
            for v_ in range(v):
                ...
                with tf.device('/device:GPU:0'):
                    ...
                    a_sparse = tf.convert_to_tensor(BV, np.float32)
                    b_sparse = tf.convert_to_tensor(Z_init, np.float32)
                    # a_sparse = tf.SparseTensor(indices, values, dense_shape)

                    c = tf.compat.v1.sparse_matmul(a_sparse, b_sparse)
                    d_sparse = tf.convert_to_tensor(Z_init.T, np.float32)

                    d = tf.compat.v1.sparse_matmul(c, d_sparse)
